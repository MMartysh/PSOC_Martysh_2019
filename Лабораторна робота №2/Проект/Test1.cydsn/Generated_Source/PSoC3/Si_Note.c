...
void Si_Note_Start(void) 
{
    ...
    Si_Note_CLKEN |= Si_Note_CLKEN_MASK;
	Si_Note_CLKSTBY |= Si_Note_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Si_Note_Stop
********************************************************************************
*
* Summary:
...
void Si_Note_Stop(void) 
{
    ...
    Si_Note_CLKEN &= (uint8)(~Si_Note_CLKEN_MASK);
	Si_Note_CLKSTBY &= (uint8)(~Si_Note_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Si_Note_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Si_Note_CLKEN & Si_Note_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Si_Note_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Si_Note_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Si_Note_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Si_Note_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Si_Note_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Si_Note_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Si_Note_GetSourceRegister();
    uint16 oldDivider = Si_Note_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Si_Note_CLKEN & Si_Note_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Si_Note_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Si_Note_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Si_Note_DIV_PTR);
}


/*******************************************************************************
* Function Name: Si_Note_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Si_Note_MOD_SRC |= modeBitMask & (uint8)Si_Note_MODE_MASK;
}


/*******************************************************************************
* Function Name: Si_Note_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Si_Note_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Si_Note_MODE_MASK));
}


/*******************************************************************************
* Function Name: Si_Note_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Si_Note_MOD_SRC & (uint8)(Si_Note_MODE_MASK);
}


/*******************************************************************************
* Function Name: Si_Note_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Si_Note_GetDividerRegister();
    uint8 oldSrc = Si_Note_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Si_Note_MOD_SRC |= CYCLK_SSS;
        Si_Note_MOD_SRC =
            (Si_Note_MOD_SRC & (uint8)(~Si_Note_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Si_Note_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Si_Note_MOD_SRC & Si_Note_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Si_Note_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Si_Note_PHASE = clkPhase & Si_Note_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Si_Note_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Si_Note_PHASE & Si_Note_PHASE_MASK;
}
